...
import Regex.EREwLookarounds.Correctness -- needed for reversal of derives
  ...
  | ⟨sp1,sp2,sp3⟩ => by
    ...
    | ⟨h1,h2⟩ =>
      have : List.length sp2 + List.length sp3 = 0 := by
        ...
        linarith
      simp at this
      have eq1 := List.length_eq_zero.mp (by simp; exact this.1)
      have eq2 := List.length_eq_zero.mp (by simp; exact this.2)
      simp_all
